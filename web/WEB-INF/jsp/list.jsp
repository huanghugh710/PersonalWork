...
<head>
    ...
    <script type="text/javascript">
        ...
        function reloadIndex2(pageIndex, code) {
            ...
            document.getElementById('firstCode').value = code;
            //设置表单提交到url
            document.forms[0].action = "${ctx}/toIndex";
            //表单提交
            document.forms[0].submit();
        ...
        <div class="w960 center">
            <ul>
                ...
                <c:forEach items="${firstArticleTypes}" var="articleType">
                    <li>
                        <a title="${articleType.name}" href="#"
                           onclick="reloadIndex2(1,'${articleType.code}')">
                                ${articleType.name}</a>
                    </li>
                </c:forEach>
            </ul>
...
<div style="positon: relative; width: 960px;margin: 0px auto;">
    ...
    <div id="booksort" style="float:left;width:210px;">
        ...
        <div class="mc">
            <c:forEach items="${firstArticleTypes}" var="articleType">
                <div class="item">
					<h3>
						<b>&gt;</b><a href="#" onclick="reloadIndex2(1,'${articleType.code}')">${articleType.name}</a>
                	</h3>
				</div>
            </c:forEach>
        </div>
    ...
    <div style="float:left;width:750px;text-align:center;">
        <div>
            <%-- 				<form:form action=""  modelAttribute="article" id="searchForm">
             --%>
			<form:form action="" modelAttribute="searchVo" id="searchForm">
				...
				<!-- 二级物品类型编号 -->
				<%-- <form:select path="articleType.secondArticleCode"  onchange="reloadIndex(1)"> --%>
				<form:select path="secondCode" onchange="reloadIndex(1)">
                    <form:option value="" label="全部"/>
                    <form:options items="${secondarticleTypes}" itemValue="code" itemLabel="name"/>
                </form:select>
				<!-- 关键字属性 -->
            ...
            <div class="sales-queue" id="tabs-1" style="background-color:white;margin-top:-25px;">
                <ul class="goods-queue3">
                    ...
                    <c:forEach items="${articles}" var="article">
                        ...
                        <li id="selbgc11">
                            <dl class="item-des">
                                <dt><a href="${ctx}/item.action?id=${article.id}" title="${article.title}"><img
                                        src="${ctx}/images/article/${article.image}" width="132" height="96"/></a></dt>
                                <dd><s>原价¥: ${article.price}</s><strong>惊爆价¥: ${article.discountPrice}</strong></dd>
                                <dd><h2><a href="${ctx}/item.action?id=${article.id}"
                                           title="${article.title}">${article.title}</a></h2></dd>
                            </dl>
                        </li>
                    </c:forEach>
