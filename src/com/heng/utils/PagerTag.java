		...
		if (recordCount > 0){   //1499 / 15  = 100
			...
			if (this.pageIndex == 1){ // 首页
				...
				if (this.pageIndex == totalPage){
					...
					str.append("<a href='#' onclick="+this.submitUrl+"("+String.valueOf(pageIndex + 1)+")"+">下一页</a>");

				}
			}else if (this.pageIndex == totalPage){ // 尾页
				String tempUrl = this.submitUrl.replace(TAG, String.valueOf(pageIndex - 1));
				str.append("<a href='#' onclick="+this.submitUrl+"("+String.valueOf(pageIndex - 1)+")"+">上一页</a>");
			...
			res.append("      }else{");
			res.append("       " + this.submitUrl + "(page_size);");
			res.append("      }");
			res.append("}");
			res.append("</script>");
		...
		if (this.totalPage <= 11){
			...
			for (int i = 1; i <= this.totalPage; i++){
				if (this.pageIndex == i){
					...
					str.append("<a href='#' onclick="+this.submitUrl+"("+String.valueOf(i)+")>"+ i +"</a>");
				}
			}
		}else{
			/** 靠近首页 */
			if (this.pageIndex <= 8){
				...
				str.append("<a href='#' onclick="+this.submitUrl+"("+this.totalPage+")"+">"+ this.totalPage +"</a>");

			}
			/** 靠近尾页 */
			else if (this.pageIndex + 8 >= this.totalPage){
				String tempUrl = this.submitUrl.replace(TAG, String.valueOf(1));
				str.append("<a href='#' onclick="+this.submitUrl+"(1)"+">1</a>");
				str.append("...");
			...
			/** 在中间 */
				...
				str.append("<a href='#' onclick="+this.submitUrl+"("+String.valueOf(this.totalPage)+")"+">"+ this.totalPage +"</a>");
			}
		}
	}
